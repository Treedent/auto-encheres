 ...
 * =============
 * full example:
 * new dBug ( variable [,forceType] [,title] );
 * example:
 * new dBug ( $myVariable );
 *
 *
 ...
 *
\*********************************************************************************************************************/
namespace SYRADEV\AutoEncheres\Utils\Debug;

/**
 * Class dBug
 * @package SYRADEV\Utils\Debug\dBug
 */
class dBug {
	...
	private function getVariableName() {
		...
		for($i=count($arrBacktrace)-1; $i>=0; $i--) {
			...
			if( array_key_exists( 'function', $arrCurrent) &&
			    ( in_array( $arrCurrent['function'], $arrInclude, true ) || ( 0 !== strcasecmp($arrCurrent['function'], 'dBug' )))) {
				continue;
			}
			$arrFile = $arrCurrent;
			break;
	...
	private function varIsObject($var, $format): void {
		...
		if(is_object($var)) {
			...
			foreach($arrObjVars as $key=>$value) {
				...
				$value=(!is_object($value) && !is_array($value) && $value==='') ? '[empty string]' : $value;
